...
def load_mesh(plydata):
    ...
    length_pc = max(max(vertexs[:,0]) - min(vertexs[:,0]), max(vertexs[:,1]) - min(vertexs[:,1]))
    scale_l = 0.1 / (length_pc)
    thresh = 1.0

    floor_s = min(vertexs[:,2])-1

    floor = min(vertexs[:,2])*scale_l
    floor2 = 0.0
    top = max(vertexs[:,2])*scale_l

    faces = tri.simplices

    mesh = trimesh.Trimesh(vertices=vertexs, faces=faces, vertex_colors=colors)
    ...
    # delete_edges = edges[np.where(lengths > thresh)[0]]

    # for i in range(len(delete_edges)):
    #     p1 = delete_edges[i][0]
    #     p2 = delete_edges[i][1]
    #     faces = np.delete(faces, np.intersect1d(np.where(faces == p1)[0], np.where(faces == p2)[0]), axis = 0)
...
##############################################
    ...
    print('load mesh: ', time.time()-start)
    ...
    for i in ti.ndrange(n_vertexs):
        ...
        for k in ti.static(range(max_cols)):  
            ...
            if j != -1:
                ...
                # Spring force
                force[i] += (-0.5 + judge_v[0])*spring_Y * d * (current_dist / original_dist - 1) 
                force[j] += (0.5 + judge_v[1])*spring_Y * d * (current_dist / original_dist - 1) 
                # # # Dashpot damping
                force[i] += -v_ij.dot(d) * d * dashpot_damping * drag_damping   
                force[j] += v_ij.dot(d) * d * dashpot_damping * drag_damping 
        ...
        if delta > 0:
            force_delta = force_delta.normalized() * ti.Vector([1.0, 1.0, 0.0]) 
            x[i] += - force_delta * delta
...
def substep4():      
    for i  in ti.ndrange(n_vertexs):                
        x[i] += x_delta[i] * dt
        x_delta[i] = ti.Vector([0.0, 0.0, 0.0])             
    ...
    for i  in ti.ndrange(n_vertexs):                
        for k in ti.static(range(max_cols)):            
            ...
            if j != -1:
                ...
                scale = current_dist / original_dist
                
                if scale > 1.5:
                    scale1 = 1.5
                    x_delta[j] += d * original_dist * (scale - scale1)
...
def flattening(pcdnm, **kwargs):
    ...
    for i in trange(substeps*6):#########fps
        substep()
        current_t += dt

    for i in trange(substeps*1):#########fps
        substep3()
        current_t += dt  
        
    for i in trange(substeps*2):#########fps
        substep4()
        current_t += dt     
