...
def load_mesh(plydata):
    ...
    # global spring_Y, dashpot_damping, drag_damping, dt, rho, rho_ini, scale_l
    ...
    mesh_o3d = o3d.geometry.TriangleMesh.create_from_point_cloud_ball_pivoting(
        plydata,
        o3d.utility.DoubleVector([avg_dist/2])
        )
    ...
    #     )
    # mesh_o3d = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(plydata, depth=8, width=0, scale=1.1, linear_fit=False)[0]
    # mesh = trimesh.Trimesh(np.asarray(mesh_o3d.vertices),
...
##############################################
    ...
    # neighbors_adface1 = {p: [] for p in range(len(vertexss))}
    # neighbors_adface2 = {p: [] for p in range(len(vertexss))}
    ...
    for face in adjacency:
        ...
        # neighbors_adface1[q1].append(p1)
        # neighbors_adface2[q1].append(p2)
        # neighbors_adface1[q2].append(p2)
        # neighbors_adface2[q2].append(p1)
...
# def initialize_mass_points():
    ...
    return xin, x, neighbor, neighbor_hold, neighbor_hold_l, adjacency_faces,\
           adjacency_faces, adjacency_uns_v, adjacency_theta, adjacency_theta,\
           face_v, xp, vp, xfin, v, force, delta_id, adjacency_force_i, adjacency_force_j,\
           adjacency_angles, rho_ini, rho, x_delta, n_adface, n_vertexs, max_cols_hold,\
           max_cols, spring_Y, dashpot_damping, drag_damping, dt, substeps,floor, colors,\
           scale_l
...
def save(pcdnm, path, xin, xfin, scale_l, colors):
...
def flattening(pcdnm, **kwargs):
    ...
    global  xin, x, neighbor, neighbor_hold, neighbor_hold_l, adjacency_faces,\
    adjacency_faces, adjacency_uns_v, adjacency_theta, adjacency_theta,\
    face_v, xp, vp, xfin, v, force, delta_id, adjacency_force_i, adjacency_force_j,\
    adjacency_angles, rho_ini, rho, x_delta, n_adface, n_vertexs, max_cols_hold,\
    max_cols, spring_Y, dashpot_damping, drag_damping, dt, substeps,floor, colors,\
    ...
    xin, x, neighbor, neighbor_hold, neighbor_hold_l, adjacency_faces,\
    adjacency_faces, adjacency_uns_v, adjacency_theta, adjacency_theta,\
    face_v, xp, vp, xfin, v, force, delta_id, adjacency_force_i, adjacency_force_j,\
    adjacency_angles, rho_ini, rho, x_delta, n_adface, n_vertexs, max_cols_hold,\
    max_cols, spring_Y, dashpot_damping, drag_damping, dt, substeps,floor, colors,\
    scale_l = load_mesh(pcdnm)
    
    current_t = 0.0

    rhoini()
        
    for i in trange(substeps*60):#########fps
        substep()
        current_t += dt
    ...
    pcdi, pcdfi, index_nan = save(pcdnm, path, xin, xfin, scale_l, colors)
...
if __name__ == '__main__':
    ...
    # pcdnm=o3d.io.read_point_cloud(kwargs['work_path']+'/interpcd_downsample.ply')
    pcdnm=o3d.io.read_point_cloud('E:/paper_flatten/integrated_code/temp/interpcd_ori.ply')
    # o3d.visualization.draw_geometries([pcdnm],mesh_show_wireframe=True,mesh_show_back_face=True)
